...
Cart.prototype.delete = function(product) {
    ...
    for(var i = 0; i < this.goodsCount; i++){
        if(this.cartItems[i].product_id == product){
...
Cart.prototype.render = function() {
    ...
    for(var i = 0; i < this.goodsCount; i++) {
        ...
        productDescriptionDiv.html('<h4>'+this.cartItems[i]['product_name']+'</h4><br><a class="delete" href="#">Delete</a>');
...
Cart.prototype.refresh = function() {
    ...
    $cartDataDiv.empty();
    $cartDataDiv.append('<p>Всего товаров: '+this.goodsCount+'</p>');
    ...
    $.get({
        ...
        success: function (data) {
            if(data.goodsCount > 0) {
                ...
                empty.appendTo('.product-list-body');
            }
        },
        context: this
    });
